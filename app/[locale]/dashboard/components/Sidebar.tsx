...
export const Sidebar = ({ user, mobileMenuOpen, setMobileMenuOpen }) => {
  ...
  const { language, changeLanguage } = useLanguage(); // ✅ যোগ করুন
  ...
  const menuItems = [
    { url: 'dashboard', name: language === 'bn' ? 'ড্যাশবোর্ড' : 'Dashboard', icon: LayoutDashboard },
    { url: 'products', name: language === 'bn' ? 'পণ্য (Products)' : 'Products', icon: ShoppingBag },
    { url: 'categories', name: language === 'bn' ? 'ক্যাটাগরি' : 'Categories', icon: FolderTree },
    { url: 'brands', name: language === 'bn' ? 'ব্র্যান্ড' : 'Brands', icon: Award },
    { url: 'sizes-colors', name: language === 'bn' ? 'সাইজ ও রঙ' : 'Sizes & Colors', icon: Ruler },
    { url: 'orders', name: language === 'bn' ? 'অর্ডার' : 'Orders', icon: ListOrdered },
    { url: 'banners', name: language === 'bn' ? 'ব্যানার' : 'Banner', icon: Flag },
    { url: 'sliders', name: language === 'bn' ? 'স্লাইডার' : 'Slider', icon: Sliders },
    { url: 'pages', name: language === 'bn' ? 'পৃষ্ঠাগুলি' : 'Pages', icon: PanelTopDashed },
    { url: 'settings', name: language === 'bn' ? 'সেটিংস' : 'Settings', icon: Ruler },
  ];
  ...
  return (
    <aside
      className={`fixed top-0 bottom-0 left-0 z-50 w-64 bg-slate-900 text-white flex flex-col transition-transform duration-300 lg:translate-x-0 ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"
        }`}
    >
      ...
      <div className="p-4 border-t border-white/10 bg-slate-950/30">
        ...
        <div className="grid grid-cols-2 gap-1 p-1 bg-white/5 rounded-lg border border-white/10">
          <button
            ...
            className={`py-1.5 text-xs font-semibold rounded-md transition-all ${language === "en"
          ...
          <button
            ...
            className={`py-1.5 text-xs font-semibold rounded-md transition-all ${language === "bn"
              ? "bg-white text-slate-900 shadow"
              : "text-slate-400"
              }`}
          >
