...
const AddForm: React.FC<AddCategoryFormProps> = ({
  ...
  useEffect(() => {
    if (editingCategory) {
      ...
      if (editingCategory.image && editingCategory.image.startsWith("/")) {
        ...
        setImageBgColor(editingCategory.imageBgColor || "#F8FAFC");
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!name.trim()) {
      setError(language === 'bn' ? "ক্যাটাগরির নাম (ইংরেজি) দিন" : "Enter category name (English)");
      return;
    }
    if (!nameBn.trim()) {
      setError(language === 'bn' ? "ক্যাটাগরির নাম (বাংলা) দিন" : "Enter category name (Bangla)");
      return;
    }
    ...
    try {
      ...
      if (isEditMode && editingCategory) {
        ...
        if (data.success) {
          ...
          toast.success(language === 'bn' ? "ক্যাটাগরি আপডেট হয়েছে!" : "Category updated successfully!");
          if (onUpdateSuccess) {
            onUpdateSuccess();
          }
          handleCancel();
        ...
        if (data.success) {
          await refreshCategories();
          toast.success(language === 'bn' ? "ক্যাটাগরি সংরক্ষণ হয়েছে!" : "Category saved successfully!");
          resetForm();
          setSuccess(true);
          setTimeout(() => setSuccess(false), 3000);
        }
        ...
        {colorOptions.map((colorOpt: any) => (
          <button
            ...
            className={`w-7 h-7 rounded-full transition-all hover:scale-110 ${
              color === colorOpt.value ? "ring-2 ring-blue-500 ring-offset-2" : ""
            }`}
