...
const ColorList: React.FC<ColorListProps> = ({ colors, setColors }) => {
  ...
  const onUpdateColor = async (id: string, updatedFields: Partial<Color>) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || (language === 'bn' ? "রঙ আপডেট করতে ব্যর্থ" : "Failed to update color"));
      }
      ...
      if (data.success && data.data) {
        ...
        toast.success(data.message || (language === 'bn' ? "রঙ সফলভাবে আপডেট হয়েছে!" : "Color updated successfully!"));
        router.refresh();
      }
    } catch (error) {
      console.error("Error updating color:", error);
      toast.error(error instanceof Error ? error.message : (language === 'bn' ? "রঙ আপডেট করতে ব্যর্থ" : "Failed to update color"));
    } finally {
      setIsUpdating(null);
    }
  };
  ...
  const onDeleteColor = async (id: string) => {
    if (!id) {
      toast.error(language === 'bn' ? 'রঙ আইডি পাওয়া যায়নি' : 'Color ID not found');
      return;
    }

    // Confirm before delete
    const confirmDelete = confirm(
      language === 'bn' ? 'আপনি কি এই রঙটি ডিলিট করতে চান?' : 'Are you sure you want to delete this color?'
    );
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(data.message || (language === 'bn' ? 'রঙ ডিলিট成功了!' : 'Color deleted successfully!'));
        router.refresh();
        ...
        toast.error(data.error || (language === 'bn' ? 'রঙ ডিলিট করতে ব্যর্থ' : 'Failed to delete color'));
      }
    } catch (error) {
      console.error("Delete error:", error);
      toast.error(error instanceof Error ? error.message : (language === 'bn' ? 'রঙ ডিলিট করতে ব্যর্থ' : 'Failed to delete color'));
    } finally {
      setIsDeleting(null);
    }
  };
  ...
  const saveEditColor = async (id: string) => {
    ...
    if (!hexRegex.test(editColorHex)) {
      toast.error(language === 'bn' ? 'ভুল হেক্স কোড। ফরম্যাট: #RRGGBB' : 'Invalid hex code. Format: #RRGGBB');
      return;
    }
      ...
      <div>
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-lg font-bold text-slate-800">
            {language === 'bn' ? 'রঙ ম্যানেজমেন্ট' : 'Colors Palette Configuration'}
          </h2>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full mb-4">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={language === 'bn' ? 'রঙ খুঁজুন...' : 'Search colors...'}
              ...
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase">
                    {language === 'bn' ? 'রঙের নাম (ইংরেজি ও বাংলা)' : 'Color Name'}
                  </th>
                  <th className="px-4 py-3 text-xs font-semibold text-slate-500 uppercase">
                    {language === 'bn' ? 'হেক্স কোড' : 'Hex Code'}
                  </th>
                ...
                {currentColors.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={3} className="py-8 text-center text-sm text-slate-500">
                      {language === 'bn' ? 'কোনো রঙ পাওয়া যায়নি।' : 'No colors found.'}
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {totalPages > 1 && (
        <div className="flex justify-between items-center border-t border-slate-100 pt-4 mt-auto">
          <span className="text-xs text-slate-500">
            {language === 'bn' ? 'দেখাচ্ছে' : 'Showing'} {startIndex + 1} {language === 'bn' ? 'থেকে' : 'to'} {Math.min(startIndex + itemsPerPage, filteredColors.length)} {language === 'bn' ? 'এর মধ্যে' : 'of'} {filteredColors.length} {language === 'bn' ? 'টি' : 'entries'}
          </span>
