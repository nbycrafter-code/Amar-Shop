...
const AddSizeForm: React.FC<AddSizeFormProps> = ({ sizes, setSizes }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!sizeName.trim()) {
      setError(language === 'bn' ? "সাইজের নাম দিন" : "Enter a size value");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || (language === 'bn' ? "সাইজ সংরক্ষণ করতে ব্যর্থ" : "Failed to save size"));
      }
      ...
      if (data.success && data.data) {
        ...
        toast.success(data.message || (language === 'bn' ? "সাইজ সফলভাবে সংরক্ষণ হয়েছে!" : "Size saved successfully!"));
        
        // Clear form only on success
        setSizeName("");
      } else {
        throw new Error(language === 'bn' ? "সাইজ সংরক্ষণ করতে ব্যর্থ" : "Failed to save size");
      }
    } catch (error) {
      console.error("Error saving size:", error);
      const errorMessage = error instanceof Error ? error.message : (language === 'bn' ? "সাইজ সংরক্ষণ করতে ব্যর্থ" : "Failed to save size");
  ...
  return (
    <form onSubmit={handleSubmit} className="relative flex gap-2 mb-6">
      <input
        ...
        placeholder={language === 'bn' ? "যেমন: S, M, XL, 42" : "e.g. S, M, XL, 42"}
