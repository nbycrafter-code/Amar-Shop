// app/theme/classic/components/Header.tsx
'use client'
...
const getBengaliSlug = (text: string) => {
  if (!text) return '';
...
// ডিকোডিং ফাংশন
const decodeBengaliText = (encodedText: string) => {
...
export function Header({
  ...
  const primaryColor = settings?.primaryColor || theme?.settings?.primaryColor || "#ef553f";
  const secondaryColor = settings?.secondaryColor || theme?.settings?.secondaryColor || "#10B981";
  ...
  const getCategoryName = (category: any) => language === 'bn' ? category.nameBn : category.name;
  const getSubCategoryName = (subcategory: any) => language === 'bn' ? subcategory.nameBn : subcategory.name;

  // toBn function for Bengali number formatting
      ...
      >
        <div className="max-w-7xl mx-auto px-4 py-3">
          ...
          <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2.5 flex-shrink-0">
                  ...
                  <div>
                    <div className="font-extrabold text-xl leading-tight text-gray-900">
                      PetShop<span style={{ color: primaryColor }}>.com.bd</span>
                    </div>
                    <div className="text-xs text-gray-400 leading-tight">
                      {language === 'bn' ? 'বাংলাদেশের পোষা প্রাণীর দোকান' : 'Bangladesh\'s Pet Shop'}
                    </div>
                  </div>
                </div>
              )}
            ...
            <form
              onSubmit={onSearch}
              className="hidden md:flex flex-1 max-w-xl items-center overflow-hidden rounded-xl border bg-white transition-all focus-within:border-emerald-400 focus-within:bg-white"
            ...
            >
              ...
              <input
                ...
                placeholder={language === 'bn' ? "কুকুর, বিড়াল, মাছের খাবার বা পণ্য খুঁজুন…" : "Search for dogs, cats, fish food or products…"}
                className="min-w-0 flex-1 px-4 py-2.5 text-sm outline-none bg-transparent"
            ...
            {/* Right actions */}
            <div className="flex items-center gap-2 flex-shrink-0 text-sm" style={{ color: textColor }}>
              {/* Mobile: Search toggle */}
              <button
                className="md:hidden p-2.5 rounded-xl transition-colors hover:bg-gray-100"
              ...
              {session?.user ? (
                <div className="relative" ref={accountRef}>
                  <button
                    ...
                    className="hidden md:flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm font-medium transition-all"
                    style={{
              ...
              >
                ...
                {wishlistCount > 0 && (
                  <span
                    className="absolute top-1 right-1 w-4 h-4 text-white text-[10px] rounded-full flex items-center justify-center font-bold leading-none"
                    style={{ backgroundColor: primaryColor }}
                  >
                    {toBn(wishlistCount)}
                  </span>
          ...
          {mobileSearchOpen && (
            <form
              onSubmit={onSearch}
              className="mt-3 flex md:hidden items-center overflow-hidden rounded-xl border bg-white"
              style={{ borderColor: borderColor }}
            >
              <select
                value={selectedCategory}
              ...
              <input
                ...
                className="min-w-0 flex-1 px-3 py-2.5 text-sm outline-none bg-transparent"
                style={{ color: textColor }}
                dir={language === 'bn' ? "auto" : "ltr"}
              />
              <button
        ...
        <div className="fixed inset-0 z-[100] flex">
          ...
          >
              ...
              {mobileCategoryOpen && mobileActiveCategory ? (
                  ...
                  <div className="px-5 py-4">
                    ...
                    <div className="space-y-1">
                      {mobileActiveCategory.subCategories &&
                        ...
                        mobileActiveCategory.subCategories.map((sub: any) => (
                          <Link
                            key={sub._id}
                ...
                <div className="divide-y" style={{ divideColor: borderColor }}>
                  ...
                  {[
                    ...
                    { href: "/categories", label: language === 'bn' ? 'ক্যাটাগরি' : 'Categories' },
                    { href: "/products", label: language === 'bn' ? 'পণ্য' : 'Products' },
                    { href: "/offers", label: language === 'bn' ? 'সেরা ডিল' : 'Top Deals' },
                    { href: "/new-arrival", label: language === 'bn' ? 'নতুন আগমন' : 'New Arrival' },
                  ].map((item) => (
                    <Link
                      key={item.href}
            ...
            <div className="border-t px-5 py-5" style={{ borderTopColor: borderColor, backgroundColor: hoverBg }}>
              <Link
                href={language === 'bn' ? "/bn/my-account/dashboard" : "/my-account/dashboard"}
                onClick={closeMobileMenu}
