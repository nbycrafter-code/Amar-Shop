...
const AddColorForm: React.FC<AddColorFormProps> = ({ colors, setColors }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Validation
    if (!colorName.trim()) {
      setError(isBn ? 'রঙের নাম (ইংরেজি) দিন' : 'Enter color name (English)');
      return;
    }
    
    if (!colorNameBn.trim()) {
      setError(isBn ? 'রঙের নাম (বাংলা) দিন' : 'Enter color name (Bangla)');
      return;
    }
    
    if (!colorHex.trim()) {
      setError(isBn ? 'রঙের হেক্স কোড দিন' : 'Enter color hex code');
      return;
    }
    ...
    if (!hexRegex.test(colorHex)) {
      setError(isBn ? 'ভুল হেক্স কোড। ফরম্যাট: #RRGGBB' : 'Invalid hex code. Format: #RRGGBB');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || (isBn ? "রঙ সংরক্ষণ করতে ব্যর্থ" : "Failed to save color"));
      }
      ...
      if (data.success && data.data) {
        ...
        toast.success(data.message || (isBn ? "রঙ সফলভাবে সংরক্ষণ হয়েছে!" : "Color saved successfully!"));
        ...
        throw new Error(isBn ? "রঙ সংরক্ষণ করতে ব্যর্থ" : "Failed to save color");
      }
    } catch (error) {
      console.error("Error saving color:", error);
      const errorMessage = error instanceof Error ? error.message : (isBn ? "রঙ সংরক্ষণ করতে ব্যর্থ" : "Failed to save color");
